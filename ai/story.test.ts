...
describe('validateChoices', () => {
  ...
  it('returns true for valid choices', () => {
    const validChoices = ['Follow the butterfly', 'Open the door', 'Talk to the owl'];
  ...
  it('logs warning for placeholder choices', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[WARN] Detected placeholder choices that may indicate LLM hallucination:', [
      'Choice A',
      'Choice C',
    ]);
  });
